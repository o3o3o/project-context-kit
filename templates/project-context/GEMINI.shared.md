...
## 🚀 CONTEXT SYNC (/ctx-load)

Use `/ctx-load` when you need to recover durable project or task context. It is not required as the first step of every session.
...
3.  **Activate Skill When Needed**: Call `activate_skill(name="context-load")` to reconstruct the task state when the current session needs it.
...
5.  **Greet**: *"Project Context loaded. Tier 1 (Project) and Tier 2 (Task) synchronized. Ready."* when a load actually happened.
