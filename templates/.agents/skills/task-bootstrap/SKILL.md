...
## Purpose
Create a fresh `.ai-governance/docs/task/active/` directory with all required state files.
...
2. **Create the directory** `.ai-governance/docs/task/active/`.

3. **Create the following files** by copying from `.ai-governance/docs/task/_template/` if it exists, otherwise create from scratch:
...
4. **Confirm** to the user: *"Active task created at .ai-governance/docs/task/active/. I've read the task and I'm ready to start work."*

## Constraints
- Do not overwrite `.ai-governance/docs/task/active/` without explicit user confirmation.
